...
export default function Game2048() {
  ...
  const doMove = useCallback((dir) => {
    ...
    const g = addRandom(newGrid);
    const newScore = score + gained;
    setGrid(g);
    setScore(newScore);
    if (newScore > best) setBest(newScore);
    if (!keepPlaying && hasWon(g) && !won) {
      setWon(true);
    }
          ...
          {grid.flat().map((val, i) => {
            ...
            return (
              <div key={i} style={{
                ...
                background: val === 0 ? "#f1f5f9" : s.bg,
                border: val === 0 ? "none" : `2px solid ${s.border}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: val >= 1024 ? "1rem" : val >= 128 ? "1.2rem" : "1.5rem",
                fontWeight: 900, color: val === 0 ? "transparent" : s.color,
                transition: "all 0.1s ease",
                boxShadow: val > 0 ? "0 2px 6px rgba(0,0,0,0.08)" : "none",
              }}>
                {val > 0 ? val : ""}
              </div>
            );
